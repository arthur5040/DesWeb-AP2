...
function exec() {
    ...
    async function asyncGetAllPlayers() {
        const response = await fetch("https://botafogo-atletas.mange.li/2024-1/all");
    ...
    async function asyncGetMasculinePlayers() {
        const response = await fetch("https://botafogo-atletas.mange.li/2024-1/masculino");
        const playersArray = await response.json();
        removeAllPlayersOnDisplay();
    ...
    async function asyncGetFemininePlayers() {
        const response = await fetch("https://botafogo-atletas.mange.li/2024-1/feminino");
        const playersArray = await response.json();
        removeAllPlayersOnDisplay();
        playersArray.forEach(createCardPlayer);
    }
    ...
    function filterPlayersByName() {
        ...
        Array.from(childs).forEach((cardPlayer) => {
            ...
            if (!cardPlayer.childNodes[1].textContent.toLowerCase().includes(inputNamePlayer.value.toLowerCase())) {
                cardPlayer.style.display = "none";
            }
            // se for igual, então aparece na tela.
            else if (cardPlayer.childNodes[1].textContent.toLowerCase().includes(inputNamePlayer.value.toLowerCase())) {
